...
```text
MySQL：视频源、录像索引、AI规则、任务、事件、处理状态
...
RTMP入口不使用token、API Key、数据库登记或回调校验。电脑APP不进行设备登记，只使用MAC和视频源信息生成唯一流名；
支持主动RTMP的监控摄像头可直接使用自定义唯一流名推送。`VideoSource`登记只用于设备
名称、固定地址和后续AI规则管理，不是SRS接收视频的前置条件。
...
- 实时配置按Worker能力同步，StreamSession保证每路视频只拉流、解码一次。
- 多算法按各自采样频率共享解码帧，推理繁忙时丢弃过期帧避免累积实时延迟。
- 内存环形缓存、连续命中、事件打开/关闭、冷却、截图和短视频证据公共底座。
- AI事件幂等上报、租户隔离查询、证据读取和人工复核API。
- 算法参数热更新、单帧/时序双缓存、算法并发隔离、失败熔断和自动恢复。
- 多Worker整流能力分配；缺少覆盖整路全部算法的Worker时报告未分配而不重复拉流。

尚未完成：

- 打架、安全帽、火灾等后续算法的专用配置项和结果页面。
- 打架、安全帽和火灾的实际模型及推理代码。
- AI事件列表、证据查看和人工确认前端页面。
- 75路视频的GPU压力测试和容量评估。

## 15. 推荐后续实施顺序

1. 实现视频黑屏、冻结、遮挡和模糊等CPU视频质量诊断。
2. 接入通用人员/车辆检测与跟踪底座。
3. 实现禁区入侵、越线、逗留和人数统计，共用检测、跟踪与ROI能力。
4. 实现逐路专用参数以及事件、证据和人工复核前端页面。
5. 接入安全帽检测模型。
6. 接入烟雾和火焰检测模型。
7. 接入打架检测模型并使用连续视频片段验证。
8. 从5路、20路逐步压力测试到75路。
9. 根据实际CPU/GPU负载、误报和漏报调整采样频率、阈值及硬件数量。
