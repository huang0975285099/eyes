...
- 证据缓存和时序分析缓存分离，避免为了短时高频动作分析而长期保存所有高频帧。
- 普通算法参数支持热更新；媒体地址、协商FPS或分辨率变化时才重启StreamSession。
- 每个算法有独立并发配额和按视频源熔断，慢算法不会形成无界队列或拖垮同路算法。
...
- `TemporalRealtimeAnalyzer`：已实现共享连续帧窗口接口，供打架、跌倒等时序模型使用。
