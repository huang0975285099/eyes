...
# SRS在本Compose中固定发布宿主机18889端口。若服务器保留了旧的配置，
...
case "${SRS_PUBLIC_BASE}" in
    *:18889) echo "  ✓ SRS公网播放地址: ${SRS_PUBLIC_BASE}" ;;
    *)
        echo "✗ 错误: SRS实际映射端口为18889，但配置的播放地址是 ${SRS_PUBLIC_BASE:-<空>}"
        echo "  请把当前目录.env中的MEDIA_SRS_HTTP_HOST改为 <服务器IP或域名>:18889"
        exit 1
        ;;
esac
...
echo "    - MediaService:  http://${REMOTE_IP}:18888"
echo "    - AI管理后台:     http://${REMOTE_IP}:18887/"
echo "    - AI健康检查:     http://${REMOTE_IP}:18887/health"
