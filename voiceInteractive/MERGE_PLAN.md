...
| Windows 通知 | Windows 10/11 原生通知中心 | 后台检测到持续变化时提示，点击可打开页面，托盘气泡仅作失败兜底 |
...
3. 网页每秒提交画面，全部后台摄像头保持待机。
4. 网页关闭或画面中断约 5 秒后，OpenCV 为每个启用的摄像头启动独立监控线程。
...
- 后台变化快照保存在 `data/events/`，默认保留 7 天、总量不超过 1024 MB；清理最多每小时扫描一次。
