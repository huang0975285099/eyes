...
启动时还会自动打开 `http://localhost:8765/` 摄像头页面。Python/OpenCV 直接采集 USB
摄像头，网页显示后台画面。问“你看到了什么”时，程序使用当前画面快照进行视觉分析，并
独立在本机核对人员库；问“画面里面的人是谁”时，只根据该快照与本地人脸样本的匹配结果
回答，不能确认时会明确说明。语音提问会按需启动主摄像头，因此网页关闭、持续监测关闭时
也可以拍摄本次画面。身份匹配无需开启持续人脸识别开关。在线视觉模式会把画面
发给在线模型，但本地人员姓名和人脸特征不会作为提示发给模型。
...
每次说“老叶老叶”都会创建一条新的独立对话；如果上一条对话仍在进行，会先把它标记为
“重新唤醒”后结束。说“再见”会关闭当前对话并回到等待唤醒状态。网页右侧按最新优先显示
全部对话及问答内容，可以在区域内滚动查看；记录只保存在本机的
`data/conversations.json`，最多保留最近50次对话，不会提交到Git。

查看当前摄像头画面时可以问：

> 你看到了什么

也可以问“画面里面的人是谁”。如果本地人员库中有清晰人脸样本，老叶会在同一张画面上
独立匹配并说明姓名与位置；没有可靠匹配时不会猜名字。两个问题可以合在一句话中，
例如“你看到了什么，画面里面的人是谁”。紧接着询问“左边是什么”等视觉
追问时，程序会继续使用最新画面，避免只根据上一轮文字描述猜测。
...
可以在画面下方分别选择主摄像头和辅助摄像头。页面会同时显示两路画面；为减少同一 USB 控制器
上的带宽争抢，双路画面现在由 Python/OpenCV 分别采集，再以 MJPEG 实时传给网页。默认使用
640×480、20 FPS，并启用摄像头 MJPEG 输出及单帧缓冲，避免浏览器同时打开两个同型号设备时
发生设备冲突，也避免旧帧堆积造成延迟。后台会识别持续黑帧并自动重开该设备，而不会将黑屏
误报为已连接。视觉问答、人物监测、
动态播报、人脸识别和人员视频采集只使用标记为“主摄像头”的一路。

画面下方“摄像头音频”可以分别选择监听麦克风和回答扬声器。两个 MM101S 在 Windows 中通常会
显示为普通名称和带 `2-` 前缀的名称。保存音频设备后，点击同一区域的“重启老叶”即可安全释放并
重新打开 PortAudio 麦克风和扬声器，无需手工关闭后台。
需要关闭时，可点击“程序控制”中的“关闭老叶”，或运行 `stop.ps1`。
...
当前配置中，动态人物监测、后台摄像头接管、动态画面播报和人脸识别四项启动时默认全部关闭，按需在各自板块开启。
...
确认已获得本人同意，然后点击该人员的“开始人脸采集”。弹窗会显示实时画面、倒计时和结果。
建议采集8～15秒：先正对镜头，再缓慢左右转头，并轻微抬头、低头。浏览器支持时会录制
视频并在本机自动抽帧；不支持视频录制时改为逐帧采集。背面、多人、模糊和人脸过小的帧
会跳过，并显示原因。视频（视频模式）、照片和 SQLite 特征库保存在 `data/faces/`，
不会交给在线大模型。
该目录已从Git版本控制中排除；代码仓库只保存建库逻辑，不保存人员照片、视频、姓名或
人脸特征。

每人建议保留10～18张有效样本。录入后打开人脸识别开关，画面会标出已知姓名；唤醒后可问
“王二在哪”，老叶会根据最近5秒内的本地识别结果回答画面左侧、中间或右侧。即使持续识别
开关关闭，“画面里面的人是谁”仍会对提问时的画面做一次本地匹配。人员删除后，其视频、
照片和特征会一并从本机删除。
