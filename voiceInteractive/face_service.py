...
class FaceDatabase:
    ...
    def add_person(self, name: str, aliases=None) -> dict:
        ...
        clean_aliases = [item for item in _clean_aliases(aliases or []) if item != clean_name]
...
class OpenCVFaceExtractor:
    ...
    def _start(self) -> None:
        ...
        worker = Path(__file__).resolve().parent / "face_recognition_worker.py"
        logs = worker.parent / "logs"
...
class FaceRecognitionService:
    ...
    def enroll_video(self, person_id: str, video_bytes: bytes, content_type: str) -> dict:
        ...
        importer = Path(__file__).resolve().parent / "face_video_import.py"
        try:
    ...
    def _recognize(self, image_bytes: bytes) -> None:
        try:
            ...
            for face in extracted.get("faces", []):
                vector = np.asarray(face["embedding"], dtype=np.float32)
                per_person: dict[str, tuple[str, float]] = {}
                for person_id, name, sample in samples:
                    score = float(np.dot(vector, sample))
                    previous = per_person.get(person_id)
                    if previous is None or score > previous[1]:
                        per_person[person_id] = (name, score)
                ranked = sorted(per_person.items(), key=lambda item: item[1][1], reverse=True)
                known = False
                person_id = None
                name = "未知人员"
                score = ranked[0][1][1] if ranked else 0.0
                second = ranked[1][1][1] if len(ranked) > 1 else -1.0
                if ranked and score >= self.config.face_match_threshold and score - second >= self.config.face_match_margin:
                    person_id, (name, score) = ranked[0]
                    known = True
                    current_candidates.add(person_id)
