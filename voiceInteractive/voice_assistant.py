...
def is_exit_command(text: str) -> bool:
    normalized = normalize_text(text)
    return any(phrase in normalized for phrase in ("退出", "再见", "停止"))
...
class CameraFrameStore:
    ...
    def request_shutdown(self) -> None:
        self.set_assistant_status("正在关闭语音助手")
        ...
        with sd.RawInputStream(
            ...
            while not (
                ...
                if state == "command" and is_time_command(text):
                    ...
                    wake_recognizer.Reset()
                    last_partial = ""
                elif state == "command" and is_exit_command(text):
                    print("再见。")
                    return
                elif state == "command" and is_vision_command(text):
                    if self.dashboard:
                        image_bytes = self.dashboard.store.latest_frame(
                            self.config.camera_frame_max_age_seconds
                        )
                    else:
                        image_bytes = None
                    if image_bytes is None:
                        answer = "我还没有看到画面，请先在网页中允许摄像头权限。"
                    else:
                        print("[视觉] 正在分析当前画面……")
                        if self.dashboard:
                    ...
                    print(f"[问题] {text}")
                    if self.dashboard:
                        self.dashboard.store.set_assistant_status("Ollama 正在思考")
                    try:
