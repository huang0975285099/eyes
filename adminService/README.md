...
Windows原生C++监控管理客户端，用于平台运维人员集中观看和回放录像。客户端通过
MediaService HTTP API读取在线流和录像，使用随程序分发的FFmpeg原生
...
本客户端只负责集中观看和录像回放，不提供设备、录像、抽帧或客户配置。逐路服务配置由
客户在移动管理平台完成，超级管理员在AIService后台管理客户和设备归属。

MediaService不使用token/API Key，因此客户端只需配置服务地址。生产环境仍应通过防火墙或
VPN限制`22222`、`1935`端口来源。

客户账号、视频源归属、逐路录像开关、保留天数、抽帧配置、抽帧结果和后续算法告警统一
在`AIService`的Web后台管理，不放在Viewer中。
